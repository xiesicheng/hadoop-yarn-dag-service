...
public class RMActiveServiceContext {
  ...
  //AM-Orchestration ZJY, add Node and Ports maps
  private final ConcurrentMap<NodeId, Set<ServicePort>> Node_SPs = new ConcurrentHashMap<NodeId, Set<ServicePort>>();
  ...
  /**
   * AM-Orchestration ZJY , add ServicePorts using in each NM
   * */
  ConcurrentMap<NodeId, Set<ServicePort>> getNMServicePorts(){
    return this.Node_SPs;
  }

  /**
   * AM-Orchestration ZJY, operation must be check again and again
   * */
  void updateNMServicePorts(NodeId nodeId, ServicePort SP){
    Set<ServicePort> ports = this.Node_SPs.get(nodeId);
    if (ports == null){
      ports = new HashSet<ServicePort>();
      ports.add(SP);
      Set<ServicePort> tmp_ports = this.Node_SPs.putIfAbsent(nodeId, ports);
      if (tmp_ports != null){
        tmp_ports.add(SP);
        this.Node_SPs.put(nodeId, tmp_ports);
      }
    }else{
      ports.add(SP);
      this.Node_SPs.put(nodeId, ports);
    }
  }
}
