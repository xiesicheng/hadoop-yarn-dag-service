...
public class ContainersMonitorImpl extends AbstractService implements
  ...
  protected void serviceInit(Configuration myConf) throws Exception {
    //LOG.info("**********sky********** ContainersMonitor is init.");
    this.conf = myConf;
  ...
  protected void serviceStart() throws Exception {
    //LOG.info("**********sky********** ContainersMonitor is starting.");
  ...
  private boolean isProcessTreeOverLimit(String containerId,
    ...
    //LOG.info("**********sky********** containerID is " + containerId + "currentMemUsage is "  + currentMemUsage);
  ...
  private class MonitoringThread extends Thread {
    ...
    public void run() {
      ...
      while (!stopped && !Thread.currentThread().isInterrupted()) {
         ...
         * add by sky
         * 2018/3/14
         * get the containers information from NM
         */
        long Node_OnlineC_Num = 0;
        long Node_OfflineC_Num = 0;
        long OnlineC_Claimed_CPU = 0;
        long OnlineC_Claimed_Mem = 0;
        float OnlineC_Used_CPU = 0;
        long OnlineC_Used_Mem = 0;
        long Offline_Claimed_CPU = 0;
        long Offline_Claimed_Mem = 0;
        float OfflineC_Used_CPU = 0;
        long OfflineC_Used_Mem = 0;

        //LOG.info("**********sky********** nodeID is " + context.getNodeId().getHost() +
        //        " containers number is " + trackingContainers.entrySet().size());

        // the main circle.trackingContainers saved all the current running containers information
        // on the node. the container will be put when NEW and will be pop when completed or killed.
        for (Entry<ContainerId, ProcessTreeInfo> entry : trackingContainers
          ...
          try {
             ...
             * add by sky
             * 2018/3/9
             *test logging every container's resource using and information.
             */
            Container containerCurrent = context.getContainers().get(containerId);

            /*
            LOG.info("**********sky********** containersID is " + containerId
                    + "container's mix type is " + containerCurrent.cloneAndGetContainerStatus().getMixType()
                    + "container's allocated memory resource from container's getCapability function is " + containerCurrent.cloneAndGetContainerStatus().getCapability().getMemorySize()
                    + "container's allocated cores resource is "  + containerCurrent.cloneAndGetContainerStatus().getCapability().getVirtualCores()
                    + "container's start time is " + containerCurrent.getContainerStartTime()
                    + "container's state is " + containerCurrent.getContainerState().toString()
                    + "container's allocated memory resource from container's getResource function is " + containerCurrent.getResource().getMemorySize()
                    + "container's allocated cores resource from container's getResource function is " + containerCurrent.getResource().getVirtualCores()
                    + "container's service port is " + containerCurrent.cloneAndGetContainerStatus().getServicePort()
                    + "currentVmemUsage is " + currentVmemUsage
                    + "currentPmemUsage is " + currentPmemUsage
                    + "cpuUsagePercentPerCore is " + cpuUsagePercentPerCore
                    +"**********sky**********");
             */
             ...
             * add by sky
             * 2018/3/14
             * calculate all the containers metrics here
             */
            if(containerCurrent.cloneAndGetContainerStatus().getMixType() == MixType.ONLINE) {
              // get every node's online container number
              Node_OnlineC_Num ++;
              // get every node's online container allocated memory(virtual memory)
              OnlineC_Claimed_Mem += containerCurrent.getResource().getMemorySize();
              // get every node's online container allocated CPU(in format of number)
              OnlineC_Claimed_CPU += containerCurrent.getResource().getVirtualCores();
              // get every node's online container total current used memory(virtual memory)
              OnlineC_Used_Mem += currentVmemUsage;
              // get every node's online container total current used cpu(in format of cpu usage percent)
              if (cpuUsagePercentPerCore > 0){
                OnlineC_Used_CPU += cpuUsagePercentPerCore;
              }
            }
            else if(containerCurrent.cloneAndGetContainerStatus().getMixType() == MixType.OFFLINE){
              // get every node's offline container number
              Node_OfflineC_Num ++;
              // get every node's offline container allocated memory(virtual memory)
              Offline_Claimed_Mem += containerCurrent.getResource().getMemorySize();
              //LOG.info("**********sky********** Offline_Claimed_Mem " + Offline_Claimed_Mem);
              //LOG.info("**********sky********** container current claimed memory " + containerCurrent.getResource().getMemorySize() );

              // get every node's offline container allocated CPU(in format of number)
              Offline_Claimed_CPU += containerCurrent.getResource().getVirtualCores();
              // get every node's offline container total current used memory(virtual memory)
              OfflineC_Used_Mem += currentVmemUsage;
              //LOG.info("**********sky********** OfflineC_Used_Mem " + OfflineC_Used_Mem);
              //LOG.info("**********sky********** current VmemUsage " + currentVmemUsage);

              // get every node's offline container total current used CPU(in format of cpu usage percent)
              if (cpuUsagePercentPerCore > 0){
                OfflineC_Used_CPU += cpuUsagePercentPerCore;
              }
            }
         ...
         * add by sky
         * 2018/3/9
         *test every node resource using
         */

        // B->MB 1048576=1024*1024
        //OnlineC_Claimed_Mem = OnlineC_Claimed_Mem / 1048576;
        OnlineC_Used_Mem = OnlineC_Used_Mem / 1048576;
        //Offline_Claimed_Mem = Offline_Claimed_Mem / 1048576;
        OfflineC_Used_Mem = OfflineC_Used_Mem / 1048576;

        int pmem = (int)(resourceCalculatorPlugin.getAvailablePhysicalMemorySize() / 1048576);
        int vmem = (int)(resourceCalculatorPlugin.getAvailableVirtualMemorySize() / 1048576);
        int upmem = getContainersUtilization().getPhysicalMemory();
        int uvmem = getContainersUtilization().getVirtualMemory();
        float ucpu = getContainersUtilization().getCPU();

        double NodeCurCpuUtil = 1.0 * ucpu;
        double NodeCurPmemUtil = 100.0 * upmem / pmem;
        double NodeCurVmemUtil = 100.0 * uvmem / vmem;



        LOG.info("&DataNodeID=" + context.getNodeId()
                + "&Node_OnlineC_Num=" + Node_OnlineC_Num
                + "&Node_OfflineC_Num=" + Node_OfflineC_Num
                + "&OnlineC_Claimed_CPU=" + OnlineC_Claimed_CPU
                + "&OnlineC_Claimed_Mem=" + OnlineC_Claimed_Mem
                + "&OnlineC_Used_CPU=" + OnlineC_Used_CPU
                + "&OnlineC_Used_Mem=" + OnlineC_Used_Mem
                + "&Offline_Claimed_CPU=" + Offline_Claimed_CPU
                + "&Offline_Claimed_Mem=" + Offline_Claimed_Mem
                + "&OfflineC_Used_CPU=" + OfflineC_Used_CPU
                + "&OfflineC_Used_Mem=" + OfflineC_Used_Mem
                + "&NodeCurCpuUtil=" + NodeCurCpuUtil
                + "&NodeCurPmemUtil=" + NodeCurPmemUtil
                + "&NodeCurVmemUtil=" + NodeCurVmemUtil);


        LOG.info("**********sky********** Total Resource sUsage stats in NM by all containers : "
