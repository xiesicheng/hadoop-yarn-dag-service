...
public class Client {
  ...
  public Client(Configuration conf) throws Exception  {
    this(
       "org.apache.hadoop.dockerservice.drive.service.ApplicationMaster",
      conf);
  }
  ...
  Client(String appMasterMainClass, Configuration conf) {
    ...
    LOG.info("conf str is " + conf.toString());
    opts = new Options();
  ...
  public boolean init(String[] args) throws ParseException {
    ...
    appName = cliParser.getOptionValue("appname", "DAG_SERVICE");
  ...
  public boolean run() throws IOException, YarnException {
    ...
    LOG.info("fs usi str is " + fs.getUri() + " and fs str is " + fs.toString());
    ...
    /*
    * set the container type for APPMaster, the tasks containers' type will by set in ApplicationMaster.java
    * */
    env.put("YARN_CONTAINER_RUNTIME_TYPE", "docker");
    env.put("YARN_CONTAINER_RUNTIME_DOCKER_IMAGE", "docker-base");
    //env.put("YARN_CONTAINER_PORT", "3306");
    env.put("YARN_CONTAINER_MIX_TYPE", "online");

    //AM-Orchestration ZJY , add hostname mapping
    env.put("ADD_HOST_NUM" , Integer.toString(DSConstants.HostMap.size()));
    int loop_num = 0;
    for (Map.Entry<String, String> entry : DSConstants.HostMap.entrySet()){
      env.put( "ADD_HOST_"+Integer.toString(++loop_num), entry.getKey()+ ":" +entry.getValue());
      LOG.info("##########junqing xiao LOG##########"
              + " ADD_HOST_" + loop_num + ":"
              + " Host name is: " + entry.getKey());
    }
    //env.put("ADD_HOST_NUM", "1");
    //env.put("ADD_HOST_1","yarn-node1:192.168.4.201");
//    env.put("CONFIG_SERVICE_PASSWORD",DSConstants.CONFIG_SERVICE_PASSWORD);
//    LOG.info("###########XMQ LOG###########"
//            +" CONFIG_SERVICE_PASSWORD: " + env.get("CONFIG_SERVICE_PASSWORD"));
    //env.put("ADD_HOST_1", "super01:192.168.7.70");
    //env.put("ADD_HOST_1", "xmqact-ThinkCentre-M910t-D565:192.168.1.118");
...
//          + ", appUser=" + report.getUser());
      ...
      //Junqing Xiao don't kill app with time
      /*
  ...
  private void addToLocalResources(FileSystem fs, String fileSrcPath,
    ...
    LOG.info("suffix str is " + suffix.toString());
    Path dst =
        new Path(fs.getHomeDirectory(), suffix);

    LOG.info("dst path is " + dst.toString());
